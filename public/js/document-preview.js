    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Perubahan pada fungsi viewDocumentInModal untuk mengatasi masalah CORS
window.viewDocumentInModal = function(id, version = 'latest') {
    ...
    // Gunakan proxy lokal untuk fetch data dokumen
    // daripada langsung ke MF server yang terkena CORS
    ...
    .then(data => {
        ...
        // Update judul modal dengan data yang diterima
        if (modalLabel && data && data.document) {
            const title = data.document.title || 'Preview Dokumen';
            modalLabel.textContent = title;
        }
    })
    .catch(error => {
        console.warn('Error fetching document info:', error);
        // Tidak perlu menampilkan error ke user, cukup log saja
    });
    ...
    if (previewFrame) {
        ...
        previewFrame.onerror = function() {
            ...
            if (errorContainer) {
                ...
                if (errorMessage) {
                    errorMessage.textContent = 'Terjadi kesalahan saat memuat dokumen. Silakan coba lagi.';
                }
            }
        };
    }
    ...
    window.handleSubmitRequest = function() {
        ...
        formData.append('user_email', 'guest@tugu.com');
